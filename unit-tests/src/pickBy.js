const pickBy = (object, predicate) => {
    ...
    if (!(predicate === undefined) && !(typeof predicate === 'function')) throw new Error('Second argument should be a FUNCTION!');
  
    let res = Object.assign(object);
    if (predicate === undefined) return res;
  
    for (let key in object) {
      if (!predicate(object[key])) delete res[key];
    }
